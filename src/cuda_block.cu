...
int main(int argc, char *argv[]) {
   ...
   printf("[GPGPU] DIM: %d, threadsPerBlock: %d, Total elapsed time %.2f ms\n", n, threadsPerBlock, accum / 10);
   free(matrix);
   ...
   if (info) {
      ...
      occupancy = (maxActiveBlocksPerSM * threadsPerBlock / props.warpSize) /
                  (float)(props.maxThreadsPerMultiProcessor /
                          props.warpSize);
      printf("Phase: 1\n");
      printf("maxActiveBlocksPerSM: %d, maxThreadsPerMultiProcessor: %d\n", maxActiveBlocksPerSM, props.maxThreadsPerMultiProcessor);
      printf("Launched blocks of size: %d, Theoretical occupancy: %f\n", threadsPerBlock, occupancy);

      cudaOccupancyMaxActiveBlocksPerMultiprocessor(&maxActiveBlocksPerSM,
                                                    floyd_warshall_block_kernel_phase2, threadsPerBlock,
                                                    0);
      occupancy = (maxActiveBlocksPerSM * threadsPerBlock / props.warpSize) /
                  (float)(props.maxThreadsPerMultiProcessor /
                          props.warpSize);
      ...
      cudaOccupancyMaxActiveBlocksPerMultiprocessor(&maxActiveBlocksPerSM,
                                                    floyd_warshall_block_kernel_phase3, threadsPerBlock,
                                                    0);
      occupancy = (maxActiveBlocksPerSM * threadsPerBlock / props.warpSize) /
                  (float)(props.maxThreadsPerMultiProcessor /
                          props.warpSize);
